// render.js
// The visual layer: reads G and draws the canvas, syncs the HTML HUD, and owns
// the camera, the radial action wheel, and the roster/dugout panel (the last
// three merged in from the former mobile.js, now shared by desktop and mobile).
// Drawing never modifies G.
...
function sizePitch() {
    ...
    if (window.innerWidth <= 500) {
        ...
        // and is panned vertically via cameraY (see the camera section below).
...
function drawSetupZones() {
    ...
    // Drop-target highlight when dragging from the roster panel
    if (dragHover) {
...
function _renderStatPips(elId, count, path) {
    ...
    if (!el) return;
    // updateHud() runs on every render() — including each frame while the
...
// ── updateHud ─────────────────────────────────────────────────────
// Syncs all the HTML overlay UI from G each render: the status strip (phase /
// turn / score / reroll + bribe pips), the roster panel (updateTeams), and the
// debug player editor. The canvas itself is drawn by render().
function updateHud() {
    // Phase / active label
...
// it's never clipped by panel overflow.  Shown on hover (desktop)
...
// ── roundRect ─────────────────────────────────────────────────────
// ── drawWheelOverlay ──────────────────────────────────────────────
// Called at the end of render(). Draws the radial action wheel.
...
// ════════════════════════════════════════════════════════════════════════════
// CAMERA · ACTION WHEEL · ROSTER PANEL   (merged from mobile.js)
// Shared by desktop and mobile — the panel + wheel replaced the old desktop
// sidebar/slide-panels. The wheel opens on long-press (touch) or right-click
// (desktop). Canvas pointer input itself lives in input.js.
// ════════════════════════════════════════════════════════════════════════════
// ── Camera ────────────────────────────────────────────────────────────────────
